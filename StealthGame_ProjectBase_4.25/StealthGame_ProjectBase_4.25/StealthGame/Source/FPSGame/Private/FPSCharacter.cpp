...
void AFPSCharacter::Tick(float DeltaTime)
{
		...
		{
			//Make sure player can see the other mesh rotate
			FRotator NewRot = CameraComponent->GetRelativeRotation();
			NewRot.Pitch = RemoteViewPitch * 360.0f / 255.0f; //uint8 >= 0  need to convert back
...
void AFPSCharacter::ServerFire_Implementation()
{
	...
	if (ProjectileClass)
	{
		//this setup will only work in single player
...
void AFPSCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	//Optimize bandwidth in some situations
